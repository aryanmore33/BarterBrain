...
module.exports = (io, socket) => {
    ...
    socket.on("accept-call", async (payload, ack = () => {}) => {
        try {
            ...
            const isParticipant = await callService.isParticipant(callId, socket.userId);
            if (!isParticipant) {
                ack({ success: false, error: "You are not a participant of this call." });
                return;
            }
            const { call, callerId } = await callService.acceptCall(callId, socket.userId);
    ...
    socket.on("reject-call", async (payload, ack = () => {}) => {
        try {
            ...
            const { call, callerId } = await callService.rejectCall(callId, socket.userId);
            callTimeoutManager.clear(callId);
    ...
    socket.on("offer", async (payload, ack = () => {}) => {
        try {
            ...
            const { receiverId } = await callService.logOffer(callId, socket.userId);
    ...
    socket.on("answer", async (payload, ack = () => {}) => {
        try {
            ...
            const { receiverId } = await callService.logAnswer(callId, socket.userId);
    ...
    socket.on("ice_candidate", async (payload, ack = () => {}) => {
        try {
            ...
            const { receiverId } = await callService.logIceCandidate(callId, socket.userId);
    ...
    socket.on("end_call", async (payload, ack = () => {}) => {
        try {
            ...
            const { call, receiverId } = await callService.endCall(callId, socket.userId);
            callTimeoutManager.clear(callId);
            socketEmitter.endCall(io, receiverId, {
                callId: call.id,
    ...
    socket.on("toggle_camera", async (payload, ack = () => {}) => {
        try {
            ...
            const { receiverId } = await callService.toggleCamera(callId, socket.userId, enabled);
            socketEmitter.cameraToggle(io, receiverId, {
                callId,
                from: socket.userId,
                enabled
            });
    ...
    socket.on("toggle_mic", async (payload, ack = () => {}) => {
        try {
            ...
            const { receiverId } = await callService.toggleMic(callId, socket.userId, enabled);
            socketEmitter.micToggle(io, receiverId, {
                callId,
                from: socket.userId,
                enabled
            });
    ...
    socket.on("toggle_screen_share", async (payload, ack = () => {}) => {
        try {
            ...
            const { receiverId } = await callService.toggleScreenShare(callId, socket.userId, enabled);
            socketEmitter.screenShare(io, receiverId, {
                callId,
                from: socket.userId,
                enabled
            });
    ...
    socket.on("leave_call", async (payload, ack = () => {}) => {
        try {
            ...
            const { receiverId } = await callService.leaveCall(callId, socket.userId);
            socketEmitter.userLeft(io, receiverId, {
                callId,
                from: socket.userId
            });
            ack({ success: true });
